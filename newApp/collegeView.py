...
class PendingQueryView(ListView):
   def get_queryset(self):
        return User.objects.filter(Verified=False,is_alumni=True,College=self.request.user.username)    
...
class AlumniAuthenticationView(LoginRequiredMixin,UpdateView):
    model = User
    fields = ["username","first_name","last_name","email","College","About","Work","Year_Joined","Branch","Image","Verified"]
